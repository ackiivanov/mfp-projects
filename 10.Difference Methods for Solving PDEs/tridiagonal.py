...
#-----------------------------------------------------------------------------
def multiply( a, d, c, x ):
    """Multiplies the tridiagonal matrix A with the vector x
    ...
    INPUT:
        ...
        x          - array to be multiplied by matrix A

    OUTPUT:
        y          - arrays with result of multiplication
    """
    ...
    for i in range( n ):
        if i == 0:
            y[i] = d[i] * x[i] + c[i] * x[i+1]
        elif i == n - 1:
            y[i] = a[i-1] * x[i-1] + d[i] * x[i]
        else:
            y[i] = a[i-1] * x[i-1] + d[i] * x[i] + c[i] * x[i+1]
